...
public class TestMethodsTest extends TestMethods {
    ...
    @Test
    public void testCartesianCoordinates() {
        int numCoords = 10;
        Random r = new Random();
        float min = -50;
        float max = 50;

        float[][] coordsToTest = randomCartesianCoordinates(numCoords, min, max);
        for (int i = 0; i < numCoords; i++) {
//            Util.printArray(coordsToTest[i]);
            assertTrue("At point of index " + i, coordsToTest[i][0] >= min);
            assertTrue("At point of index " + i, coordsToTest[i][1] >= min);
            assertTrue("At point of index " + i, coordsToTest[i][0] < max);
            assertTrue("At point of index " + i, coordsToTest[i][1] < max);
        }
    }
